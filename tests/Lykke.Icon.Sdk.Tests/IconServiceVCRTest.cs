...
namespace Lykke.Icon.Sdk.Tests
{
    public class IconServiceVCRTest
    {
        ...
        public const String PRIVATE_KEY_STRING =
                "2d42994b2f7735bbc93a3e64381864d06747e574aa94655c516f9ad0a74eed79";

        private readonly String _pk1;
        ...
        public IconServiceVCRTest()
        {
            ...
            _pk1 = System.Environment.GetEnvironmentVariable("PK1");
        ...
        {
            BigInteger balance = await _iconService.GetBalance(_wallet.GetAddress());
        ...
        {
            List<ScoreApi> apis = await _iconService.GetScoreApi(_scoreAddress);
        ...
        public async Task TestCall()
        {
            ...
            Call<RpcItem> call = new Call<RpcItem>.Builder()
                    .From(_wallet.GetAddress())
                    .To(_scoreAddress)
                    .Method("balanceOf")
                    .Params(@params)
            ...
            RpcItem result = await _iconService.CallAsync(call);
        ...
        public async Task TestCallWithClassParam()
        {
            ...
            Call<BalanceResponse> call = new Call<BalanceResponse>.Builder()
                    .From(_wallet.GetAddress())
                    .To(_scoreAddress)
                    .Method("balanceOf")
                    .Params(@params)
            ...
            BalanceResponse result = await _iconService.CallAsync(call);
        ...
        public async Task TestV2()
        {
            ...
            long requestId = DateTimeHelper.GetCurrentUnixTime();
            ...
            {
                BigInteger height = BigInteger.Parse(h.ToString());

                RpcObject @params = new RpcObject.Builder()
                        .Put("height", new RpcValue(height.ToString()))
                        .Build();
                var request = new Request(requestId, "icx_getBlockByHeight", @params);
                RpcItem result = await provider.Request(request, Converters.RPC_ITEM);

                Block block = Converters.BLOCK.ConvertTo(result.ToObject().GetItem("block").ToObject());
                List<ConfirmedTransaction> txs = block.GetTransactions();
                foreach (ConfirmedTransaction tx in txs)
                {
                    Console.WriteLine("addres:" + tx.GetTo());
                    Console.WriteLine("timestamp:" + tx.GetTimestamp());
                    Console.WriteLine("value:" + tx.GetValue());
                    Console.WriteLine("nonce:" + tx.GetNonce());
                }
